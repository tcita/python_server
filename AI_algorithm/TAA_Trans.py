...
class PositionalEncoding_TAA(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1, max_len=20):  # 定义一个足够大的 max_len 以容纳预期的最长序列
        super(PositionalEncoding_TAA, self).__init__()
...
class TransformerMovePredictor_TAA(nn.Module):
    def __init__(self, input_dim=6, d_model=128, nhead=8, num_encoder_layers=3,
        ...
        self.pos_encoder = PositionalEncoding_TAA(d_model, dropout, max_len=self.seq_len + 5)
...
def prepare_data_transformer_invariant(sample: dict, num_a=6, num_b=3):
    """
    数据预处理（置换不变版本）：
    使用 Z-score 替代 future_score，以保证特征对于B集合是置换不变的。
    """
    A = sample["A"]
    B = sample["B"]
    ...
    if len(A) != num_a or len(B) != num_b:
        return None, None, None  # 保持与原函数一致的返回
    ...
    # --- 处理B序列 (使用新特征) ---
    for val in B:  # 注意：这里不再需要 enumerate(B) 的索引 i
...
def train_model(train_data, epochs=300, batch_size=64, model_path="./trained/transformer_TAA.pth",
    ...
    # 初始化模型，修改input_dim为6
    model = TransformerMovePredictor_TAA(input_dim=6, d_model=d_model, nhead=nhead,
    ...
    try:
        for epoch in range(epochs):
            ...
            for i in range(0, len(train_data), batch_size):
                ...
                for sample in batch:
                    input_seq, order_tgt, pos_tgt = prepare_data_transformer_invariant(sample, num_a=num_a, num_b=num_b)
...
def _run_transformer_inference(TR_model, A_batch, B_batch, model_inference_indices, num_a, num_b, device):
    ...
    for batch_idx, original_idx in enumerate(model_inference_indices):
        ...
        pred_order_indices = sort_order(sample_order_logits).squeeze(0).cpu().numpy()
...
def Transformer_predict_TTA(A, B, model, num_a=6, num_b=3, num_permutations=None):
    ...
    """
    try:
        if not isinstance(model, TransformerMovePredictor_TAA):
...
if __name__ == "__main__":
    ...
    trans_path="./trained/transformer_TAA.pth"
