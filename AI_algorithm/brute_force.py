...
# 动态规划版
def recursive_StrategyAndScore(A, B, original_indices=None, move_sequence=None, used_indices=None, memo=None):
    if move_sequence is None:
        move_sequence = []
    if original_indices is None:
        original_indices = defaultdict(list)
    ...
    # 将状态转换为可哈希类型以便作为memo的键
    state_key = (tuple(B), tuple((key, used_indices[key]) for key in used_indices))

    if state_key in memo:
        return memo[state_key]  # 如果已经计算过这个状态，直接返回结果
    ...
    for i, b in enumerate(B):
        ...
        if current_used[b] < len(original_indices[b]):
            ...
            for a in range(0, len(A) + 1):
                insertscore, _, _, _, AA = simulate_insertion_tool(A, b, a)
                B_new = B[:i] + B[i + 1:]
    ...
    # 将当前状态的结果保存到memo中
    memo[state_key] = (max_score, best_moves)
