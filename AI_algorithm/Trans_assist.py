...
jsonfilename="json/Low_Score.json" # 请确保你的 JSON 文件路径正确
GPUDEBUG_MODE = True
...
class TransformerMovePredictor_assist(nn.Module):
    # 修改默认参数以反映新的固定长度
    def __init__(self, input_dim=1, d_model=128, nhead=8, num_encoder_layers=3,
        ...
        super(TransformerMovePredictor_assist, self).__init__()
...
# 修改训练部分
def train_model(train_data, epochs=1000, batch_size=64, model_path="./trained/transformer_move_predictor_assist.pth",
                num_a=6, num_b=3, warmup_epochs=20, lr_max=0.0001, lr_min=0.000001):
    """
    ...
    # 初始化模型
    model = TransformerMovePredictor_assist(input_dim=1, d_model=d_model, nhead=nhead,
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(train_data), batch_size):
                ...
                if epoch < warmup_epochs:
                    ...
                    # 余弦退火调度
                    new_lr = scheduler_cosine.get_lr()[0]
  # 获取当前余弦退火后的学习率
...
def Transformer_predict_assist(A, B, model, num_a=6, num_b=3): # <--- 修改 num_a 默认值
    """
    ...
    try:
        ...
        if not set(A) & set(B) and len(B) == len(set(B)):
             ...
             return [[0, 0], [1, 0], [2, 0]], 0 # 默认位置

        if not isinstance(model, TransformerMovePredictor_assist):
...
def train():
    ...
    train_model(train_data, epochs=1000, batch_size=512 ,
                model_path="./trained/transformer_move_predictor_assist.pth", # 新路径名
